...
<?php if(!empty($pmpro_invoice)) { ?>
	<?php
		$r_user= $pmpro_invoice->getUser();
		$r_level = $pmpro_invoice->getMembershipLevel();
   ...
   // inno_log_db::log_vcoin_login(-1, 95951, "confirmation php line 37, level id: " . $r_level->id . " :user id " . $r_user->ID);

    do_action("pmpro_create_membership", $r_level->id, $r_user->ID);
	?>
