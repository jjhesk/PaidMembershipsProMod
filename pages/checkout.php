...
// check if user or developer
$current_user_role = wp_get_current_user();
if ( !($current_user_role instanceof WP_User) )
return;
$current_role = $current_user->roles;  //$roles is an array
$current_role = $current_role[0];
if ($current_role == 'appuser'){
$url = admin_url();
die ("<meta http-equiv='refresh' content='3; url=$url' /><div class='checkuserordeveloper'>You already signed in as an app user.<br /> You have to sign out to register an application developer account.<br /> We are now redirecting you to your dashboard.</div>");
}

if(isset($current_user->membership_level->ID)){
$current_level = $current_user->membership_level->ID;
if($current_level == '1'){
$url = admin_url();
die ("<meta http-equiv='refresh' content='3; url=$url' /><div class='checkuserordeveloper'>You already signed in as an app developer.<br /> You have to sign out to register an user account.<br /> We are now redirecting you to your dashboard.</div>");
}
}

if($pmpro_requirebilling){
echo ("<style>#pmpro_paypalexpress_checkout{display: block !important;}</style>");
}
...
<form id="pmpro_form" class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
	...
	<thead>
		<tr>
			<th>
				<span class="pmpro_thead-msg"><?php _e('have account?', 'pmpro');?> <a href="<?php echo wp_login_url(pmpro_url("checkout", "?level=" . $pmpro_level->id))?>"><?php _e('Log in here', 'pmpro');?></a>.</span><?php _e('Account Information', 'pmpro');?>
			</th>						
		</tr>
	</thead>
	<tbody>                
	...
	<?php if(pmpro_getOption("gateway", true) == "paypal" && empty($pmpro_review)) { ?>
		...
		<tbody>
			<tr>
				<td>
					<div>
						<input type="radio" name="gateway" value="paypalexpress" <?php if($gateway == "paypal" || $gateway == "paypalexpress") { ?>checked="checked"<?php } ?> />
	...
	<table id="pmpro_billing_address_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if(!$pmpro_requirebilling || $gateway == "paypal" || $gateway == "paypalstandard" || $gateway == "paypalexpress" || $gateway == "twocheckout") { ?>style="display: none;"<?php } ?>>
	<thead>
		<tr>
	...
	<table id="pmpro_payment_information_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if(!$pmpro_requirebilling || $gateway == "paypal" || $gateway == "paypalstandard" || $gateway == "paypalexpress" || $gateway == "twocheckout") { ?>style="display: none;"<?php } ?>>
	<thead>
		<tr>
	...
	<div class="pmpro_submit">
		<?php if($pmpro_review) { ?>
			...
			<span id="pmpro_submit_span">
				...
				<input type="submit" class="pmpro_btn pmpro_btn-submit-checkout contactformsubmit" value="<?php _e('Complete Registration', 'pmpro');?> &raquo;" />
			</span>
				
		<?php } else { ?>
					
			<?php if($gateway == "paypal" || $gateway == "paypalexpress" || $gateway == "paypalstandard") { ?>
			<span id="pmpro_paypalexpress_checkout" <?php if($gateway !== "paypal" || !$pmpro_requirebilling) { ?>style="display: none;"<?php } ?>>
				<input type="hidden" name="submit-checkout" value="1" />		
				<input type="submit" class="checkoutpaypal" value="" />
			</span>
			<?php } ?>
			
			<span id="pmpro_submit_span" <?php if(($gateway == "paypalexpress" || $gateway == "paypalstandard" || $gateway == "paypal") && $pmpro_requirebilling) { ?>style="display: none;"<?php } ?>>
				<input type="hidden" name="submit-checkout" value="1" />		
				<input type="submit" class="pmpro_btn pmpro_btn-submit-checkout contactformsubmit" value="<?php if($pmpro_requirebilling) { if($gateway == "twocheckout") { _e('Submit and Pay with 2CheckOut', 'pmpro'); } else { _e('Submit and Check Out', 'pmpro'); } } else { _e('Submit and Confirm', 'pmpro');}?> &raquo;" />				
			</span>
		<?php } ?>
